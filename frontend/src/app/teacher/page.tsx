...
export default function TeacherAssignmentsPage() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">Assignments you have created across your assigned subjects/classes.</p>
          ...
          {assignments.map((a) => (
            <Link key={a.id} href={`/teacher/assignments/${a.id}`}>
              <Card className="flex h-full flex-col transition-shadow hover:shadow-md">
                ...
                <div className="flex-1 px-5 py-2 text-sm text-slate-500">
                  <p>
                    {a.subjectName} · {a.className}
                  </p>
                  <p className={isDeadlinePassed(a.deadline) ? "text-red-600" : ""}>Due {formatDate(a.deadline)}</p>
                </div>
                <div className="flex items-center justify-between border-t border-slate-100 px-5 py-3">
                  <span className="text-xs text-slate-500">{a.submissionCount} submission(s)</span>
                  <button onClick={(e) => handleDelete(a, e)} className="rounded p-1 text-red-500 hover:bg-red-50">
